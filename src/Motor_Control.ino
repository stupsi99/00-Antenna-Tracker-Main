...
void azimutControl(){
  ...
  int i_azimut = round(NewAzimut); //rounding for comparison with each other
  ...
  if(i_deltaAzimut >= 2){ //only adjust angle, if azimut difference is greater than 2 degrees
    ...
    int i_stepsIncrease = round((NewAzimut-CurrentAzimut)*aziGearRatio/(aziStepAngle*i_stepModeAzi)); //calculation of how many steps to increase (rounded to integer)
    ...
    CurrentAzimut = CurrentAzimut + ((i_stepsIncrease*aziStepAngle)/(aziGearRatio*i_stepModeAzi)); //add the azimuth change to the current azimuth
  ...
  if(i_azimut < i_currentAzimut){ //if desired azimuth is smaller than current actual azimuth
    ...
    int i_stepsIncrease = round((CurrentAzimut-NewAzimut)*aziGearRatio/(aziStepAngle*i_stepModeAzi));
    ...
    CurrentAzimut = CurrentAzimut - ((i_stepsIncrease*aziStepAngle)/(aziGearRatio*i_stepModeAzi)); //subtract the azimuth change from the current azimuth
